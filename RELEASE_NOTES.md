...
- **Training Capacity table** — bands added; gap callout for the new
  professions noting future balance pass.
...
#### Known gap (out of scope for this docs branch)

- `UNIVERSITY_CAPACITY` in `constants.py` doesn't yet include training caps
  for the new professions (Logistics Manager, Flight Crew, Seaman,
  Warehouse Manager, Lecturer, Tutor, Banking Analyst, Banking Clerk,
  Medical Orderly).  Today they appear only as starting workers; growth
  beyond the initial allocation falls through the unskilled-recruit route.
  RULES.md calls this out and flags it as a future balance pass.
