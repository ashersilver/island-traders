...
class TurnManager:
    ...
    def _action_request_training(
        ...
        if is_self_training:
            ...
            need_courses = sum(
                self.courses_needed(len(req.worker_ids)) for req in requests
            )
            all_ready = True
            for req in requests:
                if not self._ensure_training_courses(player, req):
                    ...
                    self.io.print(
                        ...
                        f"needs {self.courses_needed(len(req.worker_ids))} Course(s), Education Island has {have}. "
            ...
            self.io.print(
                f"  {count} worker(s) entered on-island training "
