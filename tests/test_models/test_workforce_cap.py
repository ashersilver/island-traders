"""Population cap on recruitable workforce (≤80% of population)."""
...
def test_workforce_cap_constant_is_eighty_percent():
    assert MAX_WORKFORCE_FRACTION_OF_POPULATION == 0.80


def test_available_unskilled_caps_workforce_at_eighty_percent_of_population():
    p = _farmer(population=100)
    # Fresh island with 0 workers: can recruit up to 80.
    assert p.workforce.count == 0
    assert p.available_unskilled == 80
...
def test_available_unskilled_subtracts_current_workforce():
    ...
    assert p.available_unskilled == 70   # 80 cap − 10 current


def test_available_unskilled_is_zero_when_workforce_at_or_above_cap():
    p = _farmer(population=100)
    p.workforce.add_workers(80, profession=Profession.UNSKILLED.value)
    ...
    assert p.workforce.count == 85
    assert p.available_unskilled == 0


def test_recruit_workers_honours_the_cap():
    p = _farmer(population=100)
    p.workforce.add_workers(75, profession=Profession.UNSKILLED.value)
    # Cap is 80; only 5 slots left even though we ask for 20.
    actually_recruited = p.recruit_workers(20)
    assert actually_recruited == 5
    assert p.workforce.count == 80
    assert p.available_unskilled == 0
